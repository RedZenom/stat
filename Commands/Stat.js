...
module.exports.execute = async(client, message, args,ayar,emoji) => {
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    ...
    MemberStats.findOne({ guildID: message.guild.id, userID: uye.id }, (err, data) => {
        ...
        embed.addField('⍣ Genel İstatistik',`\`● Genel Toplam Ses:\` **__${client.convertDuration(data.totalVoiceStats || 0)}__**\n\`● Genel Toplam Chat:\` ** ${data.totalChatStats || 0} mesaj**`);
        embed.addField('⍣ Haftalık Ses',`\`● Toplam:\`  **__${client.convertDuration(haftalikSesToplam)}__** \n ${haftalikSesListe}`);
        embed.addField('⍣ Haftalık Chat',`\`● Toplam:\`  **__${haftalikChatToplam}__ mesaj** \n ${haftalikChatListe}`);
